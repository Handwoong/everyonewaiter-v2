package com.handwoong.everyonewaiter.config.p6spy;
...
public class P6SpyFormatter implements MessageFormattingStrategy {
    ...
    private String format(final String category, final String sql) {
        if (isFormat(category)) {
            return createFormatSql(sql);
        }
        return sql;
    }
    ...
    private String createFormatSql(final String sql) {
        ...
        if (FormatType.isDDL(trimmedSQL)) {
            return formatSqlWithStyle(DDL, sql);
        }
        return formatSqlWithStyle(BASIC, sql);
    }
